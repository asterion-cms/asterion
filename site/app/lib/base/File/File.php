...
class File
{
     ...
     */
    public static function uploadUrl($url, $objectName, $uploadName)
    {
        if (url_exists($url)) {
     ...
     * Upload a file using the field name.
     */
    public static function upload($objectName, $name, $uploadName = '')
    {
     ...
     */
    public static function saveFile($file, $content, $tiny = false)
    {
        @touch($file);
        if (file_exists($file)) {
            if ($tiny == true) {
