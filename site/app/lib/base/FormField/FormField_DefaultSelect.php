...
class FormField_DefaultSelect
{
    ...
    public static function create($options)
    {
        ...
        $checkbox = (isset($options['checkbox']) && $options['checkbox'] != false) ? FormField_Checkbox::create(['name' => $options['name'] . '_checkbox', 'value' => $checkboxValue, 'class' => 'checkBoxInline']) : '';
        $classCheckbox = (isset($options['checkbox']) && $options['checkbox'] != false) ? 'select_checkbox' : '';
        ...
        if (is_array($value) && count($value) > 0) {
            ...
            switch ($layout) {
                default:
                    return '<div class="select form_field ' . $class . ' ' . $classCheckbox . ' ' . $errorClass . '">
    ...
    public static function loadQueryValues($query)
    {
        $result = [];
