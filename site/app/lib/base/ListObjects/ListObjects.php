...
class ListObjects
{
     ...
     */
    public function __construct($objectName, $options = [])
    {
    ...
    public function populate()
    {
        ...
        if ($this->query != '') {
            ...
            $this->list = $this->object->readList($this->options);
        }
    }
     ...
     */
    public function showList($options = [], $params = [])
    {
     ...
     */
    public function showListPager($options = [], $params = [])
    {
        $pager = $this->pager($options);
        $showResults = (isset($options['showResults'])) ? $options['showResults'] : true;
        $list_results = ($showResults) ? '<div class="list_results">' . str_replace('#RESULTS', $this->countTotal(), __('listTotal')) . '</div>' : '';
    ...
    {
        $pageUrl = (__('pageUrl') != 'pageUrl') ? __('pageUrl') : ASTERION_PAGER_URL_STRING;
